...
export async function getStats() {
  try {
    const [elementRes, userRes] = await Promise.all([
      ...
      fetch(`${BASE_URL}/api/stats/users`, { cache: "no-store" }),
    ]);
...
export async function getTopLikedElements() {
  const res = await fetch(`${BASE_URL}/api/element/top-liked`, {
    cache: "no-store", // always fetch fresh
  });
  
  if (!res.ok) {
    console.error("Failed to fetch top liked elements");
    return [];
  }
